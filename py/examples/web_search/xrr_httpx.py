...
Identical shape to the current_time example: xrr's HTTP adapter
(``xrr.adapters.http``) defines the on-disk cassette format but does
not ship a httpx transport. This ~80 LOC module fills that gap so the
Anthropic Python SDK can be pointed at an xrr-wrapped HTTP transport,
and so the same approach transparently records the Tavily call.
...
def load_session(cassette_dir: str, mode_env: str | None) -> Session:
    """Default-replay: a fresh checkout MUST run against the vendored
    cassettes with no network and no API keys.
    """
...
def new_http_client(cassette_dir: str) -> httpx.Client:
    """Build the httpx.Client the Anthropic SDK + Tavily call both share."""
